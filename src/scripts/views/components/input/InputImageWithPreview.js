...
class InputImageWithPreview extends LitWithoutShadowDom {
  ...
  _updatePhotoPreview() {
    const storyImgChange = document.querySelector('#validationStoryImageImgChange');
    const storyImgInput = document.querySelector('#validationStoryImage');

    let storyPhotoImg = null;
    if (this.defaultImage) {
      storyPhotoImg = document.querySelector('#validationStoryImageImg');
    }
    ...
    reader.onload = (event) => {
      if (this.defaultImage) {
        storyPhotoImg.classList.add('d-none');
      }
